...
#' @import checkmate
#' @export
writeReport.MDSAnalysisReport = function(report, sub.dir = "Data_Report", save.mode = TRUE, override = FALSE){
  ...
  tryCatch({
    ...
    writeLines("The following data frame shows the created dimensions for numeric columns from the dataset:", con = report.con)
    ...
    writeLines(paste0("dt = DT::datatable(", report$report.id, "$mds.analysis$mds.result.data, class = 'compact', filter = 'bottom', options = list(pageLength = 10),
    ...
    writeLines(writeRChunkOptions(chunkname = "showMDSPlot", id = getId(report),
      options = list(echo = FALSE, message = FALSE, warning = FALSE, out.width = "'90%'")), con = report.con)
