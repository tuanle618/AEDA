...
plotDens = function(data, target, col = NULL, show.plot = FALSE, ...) {
  #add.args = list(...)
  ...
  if (!is.null(col)) {
    ...
    plot = ggplot(data, a) + geom_density(aes(y = ..density..), alpha = 0.4, colour = "black", ...)
    plot = list(plot = plot)
    names(plot) = col
  } else {
    ...
    plot = lapply(1:no.num, FUN = function(y) {
      col = num[y]
      a = aes_string(x = col, colour = target)
      subplot = ggplot(data, a) + geom_density(aes(y = ..density..), alpha = 0.4, colour = "black", ...)
      return(subplot)
    })
    names(plot) = num
  }
